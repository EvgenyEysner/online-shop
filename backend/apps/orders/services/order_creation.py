...
class OrderCreationService:
    """Erstellt Orders aus einem Checkout-Payload. Idempotent über stripe_session_id."""
    ...
    def create_from_payload(
        *,
        ...
        try:
            ...
            # Ein paralleler Webhook-Aufruf (Stripe liefert Events
            # at-least-once) hat die Order zwischen unserem Existenz-Check
            # und dem .create() bereits angelegt. Der unique-Constraint auf
            # stripe_session_id garantiert, dass hier nur einer gewinnt –
            # wir laden einfach die inzwischen existierende Order.
            existing = OrderCreationService._get_existing(stripe_session_id)
            if existing is None:
                raise  # anderer Grund für den IntegrityError, nicht schlucken
            return OrderCreationService._update_existing(
                existing,
                payment_status=payment_status,
                stripe_payment_intent_id=stripe_payment_intent_id,
