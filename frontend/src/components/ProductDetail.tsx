...
interface ProductDetailProps {
  ...
  onBack: () => void;
  onAddToCart: (product: ProductDetailData, qty: number) => void;
}
...
const SPECS_BY_CATEGORY: Record<string, Array<{ label: string; value: string }>> = {
  pv: [
    { label: "Zelltyp", value: "Monokristallin PERC" },
    { label: "Modulwirkungsgrad", value: "21,4 %" },
    { label: "Leerlaufspannung (Voc)", value: "49,6 V" },
    { label: "Kurzschlussstrom (Isc)", value: "10,72 A" },
    { label: "Temperaturkoeffizient (Pmax)", value: "−0,34 % / °C" },
    { label: "Abmessungen", value: "1.722 × 1.134 × 35 mm" },
    { label: "Gewicht", value: "21,8 kg" },
    { label: "Schutzklasse", value: "IP68" },
    { label: "Garantie Leistung", value: "10 Jahre Produkt / 25 Jahre Leistung" },
    { label: "Zertifizierung", value: "IEC 61215, IEC 61730, CE" },
  ],
  battery: [
    { label: "Kapazität", value: "10,1 kWh" },
    { label: "Nutzbare Kapazität", value: "9,6 kWh" },
    { label: "Wirkungsgrad", value: "97 %" },
    { label: "Zyklenzahl", value: "> 6.000 Zyklen" },
    { label: "Anschluss", value: "Dreiphasig" },
    { label: "Schutzklasse", value: "IP55" },
    { label: "Garantie", value: "10 Jahre" },
  ],
  parts: [
    { label: "Material", value: "Kupfer, verzinnt" },
    { label: "Querschnitt", value: "6 mm²" },
    { label: "Nennspannung", value: "1.500 V DC" },
    { label: "Temperaturbereich", value: "−40 °C bis +90 °C" },
    { label: "Norm", value: "EN 50618 / IEC 62930" },
    { label: "UV-Beständigkeit", value: "25 Jahre" },
  ],
};
...
export function ProductDetail({ product, onBack, onAddToCart }: ProductDetailProps) {
  ...
  const specs = SPECS_BY_CATEGORY[product.category] ?? SPECS_BY_CATEGORY["pv"];
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-10">
          ...
          <div>
            <div className="relative rounded-xl overflow-hidden bg-muted" style={{ aspectRatio: "4/3" }}>
              ...
              />
              {product.badge && (
                <span className="absolute top-4 left-4 px-3 py-1 bg-accent text-primary text-xs font-bold rounded">
                  {product.badge}
                </span>
              )}
              {product.watt && (
                <span className="absolute bottom-4 right-4 px-3 py-1.5 bg-primary/80 backdrop-blur-sm text-white text-sm font-bold rounded-lg" style={{ fontFamily: "var(--font-display)" }}>
                  {product.watt}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-4 gap-2 mt-2">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className={`rounded-lg overflow-hidden border-2 transition-colors cursor-pointer ${i === 1 ? "border-accent" : "border-border hover:border-accent/40"}`} style={{ aspectRatio: "4/3" }}>
                  <ImageWithFallback src={product.image} alt="" className="w-full h-full object-cover opacity-80" />
                </div>
              ))}
            </div>
          </div>
            ...
            <div>
              <p className="text-accent text-xs font-semibold uppercase tracking-wider mb-2">
                {product.category === "pv" ? "Solarmodule & Wechselrichter" :
                 product.category === "battery" ? "Batteriespeicher" : "Elektroteile"}
              </p>
              <h1 className="text-foreground mb-3 leading-snug" style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(1.3rem, 3vw, 1.8rem)" }}>
                {product.name}
              </h1>
              ...
              <div className="flex items-end gap-3 mb-5">
                <span className="text-foreground" style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "2rem" }}>
              ...
              <p className="text-muted-foreground text-xs mb-5">Alle Preise inkl. MwSt. | Versandkosten ab 4,90 €</p>

              {/* Quantity + Add to cart */}
              <div className="flex items-center gap-3 mb-5">
                <div className="flex items-center border border-border rounded-lg overflow-hidden">
                  <button onClick={() => setQty(Math.max(1, qty - 1))} className="px-3 py-2.5 text-foreground hover:bg-muted transition-colors text-sm font-bold">−</button>
                  <span className="px-4 py-2.5 text-foreground font-semibold text-sm min-w-[2.5rem] text-center border-x border-border" style={{ fontFamily: "var(--font-display)" }}>{qty}</span>
                  <button onClick={() => setQty(qty + 1)} className="px-3 py-2.5 text-foreground hover:bg-muted transition-colors text-sm font-bold">+</button>
                </div>
                <button
                ...
                >
                  ...
                  {addedPulse ? "Hinzugefügt!" : `In den Warenkorb (${(product.price * qty).toLocaleString("de-DE", { style: "currency", currency: "EUR" })})`}
                </button>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  const Icon = b.icon;
                  return (
                    <div key={b.label} className="flex flex-col items-center gap-1 p-2.5 bg-muted/50 rounded-lg border border-border text-center">
        ...
        {isPVProduct && (
          <div className="bg-primary rounded-2xl p-6 md:p-8 mb-8 text-white">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-white" style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "1.2rem" }}>
                  PV-Ertragsrechner
                </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-5">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-accent font-bold" style={{ fontFamily: "var(--font-display)", fontSize: "1.1rem" }}>
                ...
                {/* Orientation */}
                <div>
                  <label className="text-white/80 text-sm font-semibold block mb-2">Ausrichtung</label>
                ...
                <div>
                  ...
                  >
                    {Object.keys(SUNHOURS_BY_REGION).map((r) => (
                      <option key={r} value={r} className="text-foreground bg-card">{r} ({SUNHOURS_BY_REGION[r]} Sonnenstunden/Jahr)</option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-accent font-bold">{electricityPrice.toFixed(2)} €/kWh</span>
                  </div>
                  <input
                    type="range"
                    min={0.20}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { icon: Zap, label: "Jahresertrag", value: `${annualKwh.toLocaleString("de-DE")} kWh`, color: "text-amber-300" },
                    { icon: Euro, label: "Jahreseinsparung", value: `${annualSavings.toLocaleString("de-DE")} €`, color: "text-green-400" },
                    { icon: Leaf, label: "CO₂ eingespart", value: `${co2Saved.toLocaleString("de-DE")} kg`, color: "text-emerald-400" },
                    { icon: TrendingUp, label: "Amortisation", value: `${amortizationYears} Jahre`, color: "text-blue-300" },
                  ].map((kpi) => {
                    const Icon = kpi.icon;
                    return (
                      <div key={kpi.label} className="bg-white/10 rounded-xl p-4">
                        <Icon size={18} className={`${kpi.color} mb-2`} />
                        <div className="text-white font-bold mb-0.5" style={{ fontFamily: "var(--font-display)", fontSize: "1.15rem" }}>
                          {kpi.value}
                        </div>
                  ...
                  <ResponsiveContainer width="100%" height={130}>
                    <AreaChart data={monthlyChartData}>
                      ...
                      <Tooltip
                        ...
                        contentStyle={{ background: "#0B2545", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, fontSize: 11, color: "#fff" }}
                      />
                      <Area type="monotone" dataKey="kwh" stroke="#F5A623" strokeWidth={2} fill="url(#cfgGrad)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-white/70 text-xs leading-relaxed">
                        {moduleCount} × {product.name.split(" ").slice(0, 3).join(" ")} = <strong className="text-white">{investmentCost.toLocaleString("de-DE", { style: "currency", currency: "EUR" })}</strong> (inkl. MwSt.)<br />
                        Einsparung Jahr 1: <strong className="text-accent">{annualSavings.toLocaleString("de-DE")} €</strong> · Amortisation: <strong className="text-white">{amortizationYears} Jahre</strong>
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-card border border-border rounded-xl mb-8">
          ...
          >
            ...
            {showSpecs ? <ChevronUp size={18} className="text-muted-foreground" /> : <ChevronDown size={18} className="text-muted-foreground" />}
          </button>
          {showSpecs && (
            <div className="px-6 pb-6 border-t border-border">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
                {specs.map((s, i) => (
                  <div key={s.label} className={`flex items-start justify-between py-2.5 ${i < specs.length - 1 ? "border-b border-border" : ""}`}>
                    <span className="text-muted-foreground text-xs">{s.label}</span>
                    <span className="text-foreground text-xs font-semibold text-right ml-4">{s.value}</span>
                  </div>
                ))}
              </div>
            </div>
